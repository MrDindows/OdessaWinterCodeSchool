...
void build(int cur, int l, int r)
{
    ...
    else
    {
        ...
        build(dcur,l,m);
        build(dcur+1,m+1,r);
        std::merge(arr[dcur], arr[dcur] + m - l + 1, arr[dcur+1], arr[dcur+1] + r - m, arr[cur]);
    }
}
int k;
int parts[50];
...
void get(int cur, int l, int r, int x, int y)
{
    if (x<l) x = l;
    if (y>r) y = r;
    if (x==l && y==r)
    ...
    else
    {
        int m = (l+r)>>1;
        int dcur = (cur<<1);
        if (x<=m) get(dcur,l,m,x,y);
        if (y>m) get(dcur+1,m+1,r,x,y);
    }
}
int main()
{
    int n,m;
    const int plus = 1000000000;
    scanf("%d%d",&n,&m);
    for (int i=1;i<=n;i++)
    {
        int x;
    ...
    for (int i=0;i<m;i++)
    {
        ...
        while(l<r)
        {
            ...
            {
                int cur = parts[i];
                s += lower_bound(arr[cur], arr[cur] + psize[i], m) - arr[cur];
            }
            if (s>=k)
            {
                r = m;
